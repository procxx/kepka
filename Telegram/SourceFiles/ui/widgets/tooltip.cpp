...
void Tooltip::popup(const QPoint &m, const QString &text, const style::Tooltip *st) {
	...
	qint32 addw = 2 * st::lineWidth + _st->textPadding.left() + _st->textPadding.right();
	qint32 addh = 2 * st::lineWidth + _st->textPadding.top() + _st->textPadding.bottom();
	...
	qint32 maxh = addh + (_st->linesMax * _st->textStyle.font->height);
	if (s.height() > maxh) {
		s.setHeight(maxh);
	}
...
void Tooltip::Show(qint32 delay, const AbstractTooltipShower *shower) {
	if (!TooltipInstance) {
		new Tooltip();
	}
