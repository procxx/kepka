...
QString IdFromOldKey(quint64 oldKey);

inline EmojiPtr FromOldKey(quint64 oldKey) {
	return Find(IdFromOldKey(oldKey));
}

inline int ColorIndexFromCode(quint32 code) {
	switch (code) {
...
inline int ColorIndexFromOldKey(quint64 oldKey) {
	return ColorIndexFromCode(quint32(oldKey & 0xFFFFFFFFLLU));
}
