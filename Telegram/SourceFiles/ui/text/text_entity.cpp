...
QString ExpressionSeparators(const QString &additional) {
	const auto utf8quotes = QString::fromUtf8("\xC2\xAB\xC2\xBB\xE2\x80\x9C\xE2\x80\x9D\xE2\x80\x98\xE2\x80\x99");
	return qsl("\\s\\.,:;<>|'\"\\[\\]\\{\\}\\~\\!\\?\\%\\^\\(\\)\\-\\+=\\x10") + utf8quotes + additional;
}
...
QSet<qint32> CreateValidProtocols() {
	auto result = QSet<qint32>();
...
QSet<qint32> CreateValidTopDomains() {
	auto result = QSet<qint32>();
	auto addOne = [&result](const QString &string) {
		result.insert(hashCrc32(string.constData(), string.size() * sizeof(QChar)));
	};
...
inline QChar RemoveOneAccent(quint32 code) {
	switch (code) {
...
bool CutPart(TextWithEntities &sending, TextWithEntities &left, qint32 limit) {
	if (left.text.isEmpty() || !limit) return false;

	qint32 currentEntity = 0, goodEntity = currentEntity, entityCount = left.entities.size();
...
bool textcmdStartsLink(const QChar *start, qint32 len, qint32 commandOffset) {
...
void ParseEntities(TextWithEntities &result, qint32 flags, bool rich) {
	...
	qint32 len = result.text.size(), commandOffset = rich ? 0 : len;
	bool inLink = false, commandIsLink = false;
	const QChar *start = result.text.constData(), *end = start + result.text.size();
	for (qint32 offset = 0, matchOffset = offset, mentionSkip = 0; offset < len;) {
...
QString ApplyEntities(const TextWithEntities &text) {
	...
	QMultiMap<qint32, QString> closingTags;
...
void PrepareForSending(TextWithEntities &result, qint32 flags) {
