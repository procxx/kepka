...
void CountrySelectBox::Inner::paintEvent(QPaintEvent *e) {
	...
	if (l) {
		...
		qint32 from = floorclamp(r.y() - st::countriesSkip, _rowHeight, 0, l);
		qint32 to = ceilclamp(r.y() + r.height() - st::countriesSkip, _rowHeight, 0, l);
...
void CountrySelectBox::Inner::selectSkip(qint32 dir) {
	_mouseSelection = false;
...
void CountrySelectBox::Inner::selectSkipPage(qint32 h, qint32 dir) {
	qint32 points = h / _rowHeight;
	if (!points) return;
