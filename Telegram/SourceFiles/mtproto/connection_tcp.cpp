	...
	do {
		...
		if (bytes > 0) {
			...
			if (packetLeft) {
				...
				bool move = false;
				while (packetRead >= 4) {
					quint32 packetSize = tcpPacketSize(currentPos - packetRead);
...
mtpBuffer AbstractTCPConnection::handleResponse(const char *packet, quint32 length) {
	...
	if (size * qint32(sizeof(mtpPrime)) != len) {
...
void TCPConnection::socketPacket(const char *packet, quint32 length) {
	if (status == FinishedWork) return;
