...
QStringList atosstr(quint64 *addresses, int count, quint64 base) {
	QStringList result;
	...
	while (i < l) {
		...
		for (qint32 start = i; i < l; ++i) {
			...
			if (!base) {
				...
				if (m.hasMatch()) {
					if (quint64 address = m.captured(1).toULongLong()) {
		...
		for (; i < l; ++i) {
			...
			if (QRegularExpression(qsl("^\\d+")).match(line).hasMatch()) {
				...
				if (lst.size() > 2) {
					quint64 addr = lst.at(2).startsWith(qstr("0x")) ? lst.at(2).mid(2).toULongLong(0, 16) : lst.at(2).toULongLong();
