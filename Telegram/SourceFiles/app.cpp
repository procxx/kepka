...
HistoryItem *hoveredItem = nullptr, *pressedItem = nullptr, *hoveredLinkItem = nullptr, *pressedLinkItem = nullptr,
            *contextItem = nullptr, *mousedItem = nullptr;
...
QString onlineText(TimeId online, TimeId now, bool precise) {
	...
	if (precise) {
		...
		return lng_status_lastseen_date_time(lt_date, dOnline.date().toString(qsl("dd.MM.yy")), lt_time,
		                                     dOnline.time().toString(cTimeFormat()));
	}
	qint32 minutes = (now - online) / 60;
	if (!minutes) {
...
UserData *feedUser(const MTPUser &user) {
	...
	case mtpc_user: {
		...
		if (d.is_deleted()) {
			...
			QString fname = (!minimal || noLocalName) ?
			                    (d.has_first_name() ? TextUtilities::SingleLine(qs(d.vfirst_name)) : QString()) :
			                    data->firstName;
			QString lname = (!minimal || noLocalName) ?
			                    (d.has_last_name() ? TextUtilities::SingleLine(qs(d.vlast_name)) : QString()) :
			                    data->lastName;

			QString phone = minimal ? data->phone() : (d.has_phone() ? qs(d.vphone) : QString());
			QString uname =
			    minimal ? data->username : (d.has_username() ? TextUtilities::SingleLine(qs(d.vusername)) : QString());
			...
			bool showPhoneChanged = !isServiceUser(data->id) && !d.is_self() &&
			                        ((showPhone && data->contact) || (!showPhone && !data->contact));
			if (minimal) {
			...
			QString pname = (showPhoneChanged || phoneChanged || nameChanged) ?
			                    ((showPhone && !phone.isEmpty()) ? formatPhone(phone) : QString()) :
			                    data->nameOrPhone;
		...
		if (!minimal) {
			if (d.has_bot_info_version()) {
				...
				data->botInfo->inlinePlaceholder =
				    d.has_bot_inline_placeholder() ? '_' + qs(d.vbot_inline_placeholder) : QString();
			} else {
				data->setBotInfoVersion(-1);
			}
...
PeerData *feedChat(const MTPChat &chat) {
	...
	case mtpc_channel: {
		...
		if (minimal) {
			auto mask = MTPDchannel::Flag::f_broadcast | MTPDchannel::Flag::f_verified |
			            MTPDchannel::Flag::f_megagroup | MTPDchannel::Flag::f_democracy;
		...
		if (canViewAdmins != cdata->canViewAdmins() || canViewMembers != cdata->canViewMembers() ||
...
void feedParticipants(const MTPChatParticipants &p, bool requestBotInfos) {
	...
	Notify::peerUpdatedDelayed(chat,
...
template <typename TMTPDclass> void updateEditedMessage(const TMTPDclass &m) {
	auto peerId = peerFromMTP(m.vto_id);
	if (m.has_from_id() && peerId == Auth().userPeerId()) {
		peerId = peerFromUser(m.vfrom_id);
	}
...
ImagePtr image(const MTPPhotoSize &size) {
	...
	case mtpc_photoSize: {
		...
		if (d.vlocation.type() == mtpc_fileLocation) {
			...
			return ImagePtr(
			    StorageImageLocation(d.vw.v, d.vh.v, l.vdc_id.v, l.vvolume_id.v, l.vlocal_id.v, l.vsecret.v),
			    d.vsize.v);
		}
	} break;
	case mtpc_photoCachedSize: {
		...
		if (d.vlocation.type() == mtpc_fileLocation) {
			...
			return ImagePtr(
			    StorageImageLocation(d.vw.v, d.vh.v, l.vdc_id.v, l.vvolume_id.v, l.vlocal_id.v, l.vsecret.v), bytes);
...
void feedWereDeleted(ChannelId channelId, const QVector<MTPint> &msgsIds) {
	...
	ChannelHistory *channelHistory =
	    (channelId == NoChannel) ? 0 : App::historyLoaded(peerFromChannel(channelId))->asChannelHistory();
	...
	if (main()) {
		for (QMap<History *, bool>::const_iterator i = historiesToCheck.cbegin(), e = historiesToCheck.cend(); i != e;
		     ++i) {
...
void feedUserLink(MTPint userId, const MTPContactLink &myLink, const MTPContactLink &foreignLink) {
	...
	if (user) {
		...
		case mtpc_contactLinkContact:
			...
			break;
		case mtpc_contactLinkHasPhone: user->contact = 0; break;
		case mtpc_contactLinkNone:
		case mtpc_contactLinkUnknown: user->contact = -1; break;
		}
		if (user->contact < 1) {
		...
		bool showPhoneChanged = !isServiceUser(user->id) && !user->isSelf() &&
		...
		if (showPhoneChanged) {
			user->setName(TextUtilities::SingleLine(user->firstName), TextUtilities::SingleLine(user->lastName),
			              showPhone ? App::formatPhone(user->phone()) : QString(),
...
PhotoData *feedPhoto(const MTPPhoto &photo, const PreparedPhotoThumbs &thumbs) {
	...
	for (PreparedPhotoThumbs::const_iterator i = thumbs.cbegin(), e = thumbs.cend(); i != e; ++i) {
		...
		case 'w':
			...
			newMediumLevel = 8;
			newFullLevel = 2;
			break; // box 2560x2560 // if loading this fix HistoryPhoto::updateFrom
		case 'a':
			newThumbLevel = 1;
			newMediumLevel = 4;
			newFullLevel = 8;
	...
	case mtpc_photo: {
		...
		return App::photoSet(ph.vid.v, 0, ph.vaccess_hash.v, ph.vdate.v, ImagePtr(*thumb, "JPG"),
		                     ImagePtr(*medium, "JPG"), ImagePtr(*full, "JPG"));
	} break;
...
PhotoData *feedPhoto(const MTPDphoto &photo, PhotoData *convert) {
	...
	if (thumb && medium && full) {
		return App::photoSet(photo.vid.v, convert, photo.vaccess_hash.v, photo.vdate.v, App::image(*thumb),
		                     App::image(*medium), App::image(*full));
	}
...
DocumentData *feedDocument(const MTPdocument &document, const QPixmap &thumb) {
	...
	case mtpc_document: {
		...
		return App::documentSet(d.vid.v, 0, d.vaccess_hash.v, d.vversion.v, d.vdate.v, d.vattributes.v,
		                        qs(d.vmime_type), ImagePtr(thumb, "JPG"), d.vdc_id.v, d.vsize.v,
...
DocumentData *feedDocument(const MTPdocument &document, DocumentData *convert) {
	...
	case mtpc_documentEmpty: {
		return App::documentSet(document.c_documentEmpty().vid.v, convert, 0, 0, 0, QVector<MTPDocumentAttribute>(),
		                        QString(), ImagePtr(), 0, 0, StorageImageLocation());
	} break;
	}
	return App::document(0);
}
...
DocumentData *feedDocument(const MTPDdocument &document, DocumentData *convert) {
	return App::documentSet(document.vid.v, convert, document.vaccess_hash.v, document.vversion.v, document.vdate.v,
	                        document.vattributes.v, qs(document.vmime_type), App::image(document.vthumb),
	                        document.vdc_id.v, document.vsize.v, App::imageLocation(document.vthumb));
}

WebPageData *feedWebPage(const MTPDwebPage &webpage, WebPageData *convert) {
	auto description =
	    TextWithEntities{webpage.has_description() ? TextUtilities::Clean(qs(webpage.vdescription)) : QString()};
	...
	return App::webPageSet(
	    webpage.vid.v, convert, webpage.has_type() ? qs(webpage.vtype) : qsl("article"), qs(webpage.vurl),
	    qs(webpage.vdisplay_url), siteName, webpage.has_title() ? qs(webpage.vtitle) : QString(), description,
	    webpage.has_photo() ? App::feedPhoto(webpage.vphoto) : nullptr,
	    webpage.has_document() ? App::feedDocument(webpage.vdocument) : nullptr,
	    webpage.has_duration() ? webpage.vduration.v : 0, webpage.has_author() ? qs(webpage.vauthor) : QString(), 0);
}

WebPageData *feedWebPage(const MTPDwebPagePending &webpage, WebPageData *convert) {
	return App::webPageSet(webpage.vid.v, convert, QString(), QString(), QString(), QString(), QString(),
	                       TextWithEntities(), nullptr, nullptr, 0, QString(), webpage.vdate.v);
}
...
WebPageData *feedWebPage(WebPageId webPageId, const QString &siteName, const TextWithEntities &content) {
	return App::webPageSet(webPageId, nullptr, qsl("article"), QString(), QString(), siteName, QString(), content,
	                       nullptr, nullptr, 0, QString(), 0);
}

GameData *feedGame(const MTPDgame &game, GameData *convert) {
	return App::gameSet(game.vid.v, convert, game.vaccess_hash.v, qs(game.vshort_name), qs(game.vtitle),
	                    qs(game.vdescription), App::feedPhoto(game.vphoto),
	                    game.has_document() ? App::feedDocument(game.vdocument) : nullptr);
}
...
WebPageData *webPageSet(const WebPageId &webPage, WebPageData *convert, const QString &type, const QString &url,
                        ...
                        const TextWithEntities &description, PhotoData *photo, DocumentData *document, qint32 duration,
	...
	if (convert) {
		...
		if ((convert->url.isEmpty() && !url.isEmpty()) ||
		    (convert->pendingTill && convert->pendingTill != pendingTill && pendingTill >= -1)) {
	...
	if (i == webPagesData.cend()) {
		if (convert) {
			...
			result = new WebPageData(webPage, toWebPageType(type), url, displayUrl, siteName, title, description,
			                         document, photo, duration, author, (pendingTill >= -1) ? pendingTill : -1);
			if (pendingTill > 0) {
		...
		if (result != convert) {
			if ((result->url.isEmpty() && !url.isEmpty()) ||
			    (result->pendingTill && result->pendingTill != pendingTill && pendingTill >= -1)) {
...
GameData *gameSet(const GameId &game, GameData *convert, const quint64 &accessHash, const QString &shortName,
                  const QString &title, const QString &description, PhotoData *photo, DocumentData *document) {
	if (convert) {
		if (convert->id != game) {
...
void forgetMedia() {
	lastPhotos.clear();
	lastPhotosMap.clear();
	for_const (auto photo, ::photosData) { photo->forget(); }
	for_const (auto document, ::documentsData) { document->forget(); }
	for_const (auto location, ::locationsData) { location->thumb->forget(); }
}
...
MTPPhoto photoFromUserPhoto(MTPint userId, MTPint date, const MTPUserProfilePhoto &photo) {
	if (photo.type() == mtpc_userProfilePhoto) {
		...
		photoSizes.push_back(
		    MTP_photoSize(MTP_string("a"), uphoto.vphoto_small, MTP_int(160), MTP_int(160), MTP_int(0)));
...
QString peerName(const PeerData *peer, bool forDialogs) {
	return peer ?
	           ((forDialogs && peer->isUser() && !peer->asUser()->nameOrPhone.isEmpty()) ? peer->asUser()->nameOrPhone :
	                                                                                       peer->name) :
	           lang(lng_deleted);
}
...
void historyUnregItem(HistoryItem *item) {
	...
	if (j != ::dependentItems.cend()) {
		...
		for_const (auto dependent, items) { dependent->dependencyItemRemoved(item); }
	}
...
void historyUpdateDependent(HistoryItem *item) {
	...
	if (j != ::dependentItems.cend()) {
		for_const (HistoryItem *dependent, j.value()) { dependent->updateDependencyItem(); }
	}
	if (App::main()) {
...
void historyClearMsgs() {
	...
	for_const (auto item, toDelete) { delete item; }
...
void historyClearItems() {
	...
	for_const (auto peer, ::peersData) { delete peer; }
	::peersData.clear();
	for_const (auto game, ::gamesData) { delete game; }
	::gamesData.clear();
	for_const (auto webpage, ::webPagesData) { delete webpage; }
	::webPagesData.clear();
	for_const (auto photo, ::photosData) { delete photo; }
	::photosData.clear();
	for_const (auto document, ::documentsData) { delete document; }
	::documentsData.clear();
...
void prepareCorners(RoundCorners index, qint32 radius, const QBrush &brush, const style::color *shadow = nullptr,
                    QImage *cors = nullptr) {
...
const QPixmap &emojiSingle(EmojiPtr emoji, qint32 fontHeight) {
	...
	if (i == map.cend()) {
		auto image = QImage(Ui::Emoji::Size() + st::emojiPadding * cIntRetinaFactor() * 2,
		                    fontHeight * cIntRetinaFactor(), QImage::Format_ARGB32_Premultiplied);
		if (cRetina()) image.setDevicePixelRatio(cRetinaFactor());
		image.fill(Qt::transparent);
		{
			QPainter p(&image);
			emojiDraw(p, emoji, st::emojiPadding * cIntRetinaFactor(),
			          (fontHeight * cIntRetinaFactor() - Ui::Emoji::Size()) / 2);
		}
...
bool isValidPhone(QString phone) {
	...
	return phone.length() >= 8 || phone == qsl("777") || phone == qsl("333") || phone == qsl("111") ||
	       (phone.startsWith(qsl("42")) && (phone.length() == 2 || phone.length() == 5 || phone == qsl("4242")));
}
...
void complexOverlayRect(Painter &p, QRect rect, ImageRoundRadius radius, ImageRoundCorners corners) {
	if (radius == ImageRoundRadius::Ellipse) {
		...
		auto overlayCorners =
		    (radius == ImageRoundRadius::Small) ? SelectedOverlaySmallCorners : SelectedOverlayLargeCorners;
...
void roundRect(Painter &p, qint32 x, qint32 y, qint32 w, qint32 h, style::color bg, const CornersPixmaps &corner,
               const style::color *shadow, RectParts parts) {
...
void roundRect(Painter &p, qint32 x, qint32 y, qint32 w, qint32 h, style::color bg, ImageRoundRadius radius,
               RectParts parts) {
	auto colorKey = ((quint32(bg->c.alpha()) & 0xFF) << 24) | ((quint32(bg->c.red()) & 0xFF) << 16) |
	                ((quint32(bg->c.green()) & 0xFF) << 8) | ((quint32(bg->c.blue()) & 0xFF) << 24);
