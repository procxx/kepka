...
void Reader::init(const FileLocation &location, const QByteArray &data) {
	if (threads.size() < ClipThreadsCount) {
		...
		_threadIndex = qint32(rand_value<quint32>() % threads.size());
		qint32 loadLevel = 0x7FFFFFFF;
		for (qint32 i = 0, l = threads.size(); i < l; ++i) {
			qint32 level = managers.at(i)->loadLevel();
			if (level < loadLevel) {
				_threadIndex = i;
				loadLevel = level;
			}
...
Reader::Frame *Reader::frameToShow(qint32 *index) const { // 0 means not ready
...
Reader::Frame *Reader::frameToWrite(qint32 *index) const { // 0 means not ready
	qint32 step = _step.loadAcquire(), i;
	if (step == WaitingForDimensionsStep) {
		i = 0;
	} else if (step == WaitingForRequestStep) {
		if (index) *index = 0;
...
Reader::Frame *Reader::frameToWriteNext(bool checkNotWriting, qint32 *index) const {
	qint32 step = _step.loadAcquire(), i;
...
void Reader::callback(Reader *reader, qint32 threadIndex, Notification notification) {
...
qint32 Reader::width() const {
	return _width;
}

qint32 Reader::height() const {
	return _height;
}
