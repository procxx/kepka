...
QImage createAlignedImage(QSize size) {
	...
	return QImage(alignedbuffer, width, height, bytesperline, QImage::Format_ARGB32, alignedImageBufferCleanupHandler,
	              cleanupdata);
}
...
FFMpegReaderImplementation::FFMpegReaderImplementation(FileLocation *location, QByteArray *data,
...
ReaderImplementation::ReadResult FFMpegReaderImplementation::readNextFrame() {
	...
	do {
		...
		if (res == AVERROR_EOF) {
			...
			if ((res = avformat_seek_file(_fmtContext, _streamId, std::numeric_limits<int64_t>::min(), 0,
				...
				if ((res = av_seek_frame(_fmtContext, _streamId, 0, AVSEEK_FLAG_BYTE)) < 0) {
					if ((res = av_seek_frame(_fmtContext, _streamId, 0, AVSEEK_FLAG_FRAME)) < 0) {
						if ((res = av_seek_frame(_fmtContext, _streamId, 0, 0)) < 0) {
							...
							LOG(("Gif Error: Unable to av_seek_frame() to the start %1, error %2, %3")
							        .arg(logData())
							        .arg(res)
			...
			char err[AV_ERROR_MAX_STRING_SIZE] = {0};
			LOG(("Gif Error: Unable to avcodec_receive_frame() %1, error %2, %3")
			        .arg(logData())
			        .arg(res)
			        .arg(av_make_error_string(err, sizeof(err), res)));
			return ReadResult::Error;
		...
		if (res < 0) {
			...
			char err[AV_ERROR_MAX_STRING_SIZE] = {0};
			LOG(("Gif Error: Unable to avcodec_send_packet() %1, error %2, %3")
			        .arg(logData())
			        .arg(res)
			        .arg(av_make_error_string(err, sizeof(err), res)));
...
void FFMpegReaderImplementation::processReadFrame() {
	qint64 duration = _frame->pkt_duration;
	qint64 framePts = _frame->pts;
	TimeMs frameMs = (framePts * 1000LL * _fmtContext->streams[_streamId]->time_base.num) /
	                 _fmtContext->streams[_streamId]->time_base.den;
	_currentFrameDelay = _nextFrameDelay;
	if (_frameMs + _currentFrameDelay < frameMs) {
		_currentFrameDelay = qint32(frameMs - _frameMs);
	...
	if (duration == AV_NOPTS_VALUE) {
		...
		_nextFrameDelay = (duration * 1000LL * _fmtContext->streams[_streamId]->time_base.num) /
		                  _fmtContext->streams[_streamId]->time_base.den;
	}
	_frameMs = frameMs;
...
TimeMs FFMpegReaderImplementation::durationMs() const {
	...
	return (_fmtContext->streams[_streamId]->duration * 1000LL * _fmtContext->streams[_streamId]->time_base.num) /
	       _fmtContext->streams[_streamId]->time_base.den;
}
...
bool FFMpegReaderImplementation::renderFrame(QImage &to, bool &hasAlpha, const QSize &size) {
	...
	hasAlpha =
	    (_frame->format == AV_PIX_FMT_BGRA || (_frame->format == -1 && _codecContext->pix_fmt == AV_PIX_FMT_BGRA));
	if (_frame->width == toSize.width() && _frame->height == toSize.height() && hasAlpha) {
		...
		if ((_swsSize != toSize) || (_frame->format != -1 && _frame->format != _codecContext->pix_fmt) ||
		    !_swsContext) {
			_swsSize = toSize;
			_swsContext =
			    sws_getCachedContext(_swsContext, _frame->width, _frame->height, AVPixelFormat(_frame->format),
			                         toSize.width(), toSize.height(), AV_PIX_FMT_BGRA, 0, 0, 0, 0);
		}
		// AV_NUM_DATA_POINTERS defined in AVFrame struct
		uint8_t *toData[AV_NUM_DATA_POINTERS] = {to.bits(), nullptr};
		int toLinesize[AV_NUM_DATA_POINTERS] = {to.bytesPerLine(), 0};
		int res;
		if ((res = sws_scale(_swsContext, _frame->data, _frame->linesize, 0, _frame->height, toData, toLinesize)) !=
		    _swsSize.height()) {
			LOG(("Gif Error: Unable to sws_scale to good size %1, height %2, should be %3")
			        .arg(logData())
			        .arg(res)
			        .arg(_swsSize.height()));
			return false;
	...
	if (_audioStreamId >= 0) {
		while (_frameMs + 5000 > _lastReadAudioMs && _frameMs + 15000 > _lastReadVideoMs) {
			auto packetResult = readAndProcessPacket();
...
bool FFMpegReaderImplementation::start(Mode mode, TimeMs &positionMs) {
	...
	_ioContext = avio_alloc_context(_ioBuffer, AVBlockSize, 0, static_cast<void *>(this),
	                                &FFMpegReaderImplementation::_read, 0, &FFMpegReaderImplementation::_seek);
	...
	if ((res = avformat_open_input(&_fmtContext, 0, 0, 0)) < 0) {
		...
		LOG(("Gif Error: Unable to avformat_open_input %1, error %2, %3")
		        .arg(logData())
		        .arg(res)
		        .arg(av_make_error_string(err, sizeof(err), res)));
		return false;
	...
	if ((res = avformat_find_stream_info(_fmtContext, 0)) < 0) {
		LOG(("Gif Error: Unable to avformat_find_stream_info %1, error %2, %3")
		        .arg(logData())
		        .arg(res)
		        .arg(av_make_error_string(err, sizeof(err), res)));
		return false;
	...
	if (_streamId < 0) {
		LOG(("Gif Error: Unable to av_find_best_stream %1, error %2, %3")
		        .arg(logData())
		        .arg(_streamId)
		        .arg(av_make_error_string(err, sizeof(err), _streamId)));
		return false;
	}
	...
	if ((res = avcodec_parameters_to_context(_codecContext, _fmtContext->streams[_streamId]->codecpar)) < 0) {
		LOG(("Gif Error: Unable to avcodec_parameters_to_context %1, error %2, %3")
		        .arg(logData())
		        .arg(res)
		        .arg(av_make_error_string(err, sizeof(err), res)));
		return false;
	...
	if ((res = avcodec_open2(_codecContext, _codec, 0)) < 0) {
		LOG(("Gif Error: Unable to avcodec_open2 %1, error %2, %3")
		        .arg(logData())
		        .arg(res)
		        .arg(av_make_error_string(err, sizeof(err), res)));
		return false;
	...
	if (_audioStreamId >= 0) {
		...
		if ((res = avcodec_parameters_to_context(audioContext, _fmtContext->streams[_audioStreamId]->codecpar)) < 0) {
			LOG(("Audio Error: Unable to avcodec_parameters_to_context %1, error %2, %3")
			        .arg(logData())
			        .arg(res)
			        .arg(av_make_error_string(err, sizeof(err), res)));
			return false;
		...
		if ((res = avcodec_open2(audioContext, audioCodec, 0)) < 0) {
			...
			if (_fmtContext->streams[_audioStreamId]->duration == AV_NOPTS_VALUE) {
				...
				soundData->length = (_fmtContext->streams[_audioStreamId]->duration * soundData->frequency *
				                     _fmtContext->streams[_audioStreamId]->time_base.num) /
				                    _fmtContext->streams[_audioStreamId]->time_base.den;
			}
		}
	}
	if (positionMs > 0) {
		auto ts = (positionMs * _fmtContext->streams[_streamId]->time_base.den) /
		          (1000LL * _fmtContext->streams[_streamId]->time_base.num);
...
FFMpegReaderImplementation::PacketResult FFMpegReaderImplementation::readPacket(AVPacket *packet) {
	...
	if ((res = av_read_frame(_fmtContext, packet)) < 0) {
		...
		char err[AV_ERROR_MAX_STRING_SIZE] = {0};
		LOG(("Gif Error: Unable to av_read_frame() %1, error %2, %3")
		        .arg(logData())
		        .arg(res)
		        .arg(av_make_error_string(err, sizeof(err), res)));
...
TimeMs FFMpegReaderImplementation::countPacketMs(AVPacket *packet) const {
	qint64 packetPts = (packet->pts == AV_NOPTS_VALUE) ? packet->dts : packet->pts;
	TimeMs packetMs = (packetPts * 1000LL * _fmtContext->streams[packet->stream_index]->time_base.num) /
	                  _fmtContext->streams[packet->stream_index]->time_base.den;
	return packetMs;
}
