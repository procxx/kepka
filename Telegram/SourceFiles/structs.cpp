...
quint64 PtsWaiter::ptsKey(PtsSkippedQueue queue, qint32 pts) {
	return _queue.insert(quint64(quint32(pts)) << 32 | (++_skippedKey), queue).key();
}

void PtsWaiter::setWaitingForSkipped(ChannelData *channel, qint32 ms) {
	if (ms >= 0) {
		if (App::main()) {
...
void PtsWaiter::setWaitingForShortPoll(ChannelData *channel, qint32 ms) {
	if (ms >= 0) {
		if (App::main()) {
			App::main()->ptsWaiterStartTimerFor(channel, ms);
		}
...
bool PtsWaiter::updated(ChannelData *channel, qint32 pts, qint32 count, const MTPUpdates &updates) {
...
bool PtsWaiter::updated(ChannelData *channel, qint32 pts, qint32 count, const MTPUpdate &update) {
	if (_requesting || _applySkippedLevel) {
		return true;
	} else if (pts <= _good && count > 0) {
		return false;
...
bool PtsWaiter::updated(ChannelData *channel, qint32 pts, qint32 count) {
	if (_requesting || _applySkippedLevel) {
		return true;
	} else if (pts <= _good && count > 0) {
		return false;
...
bool PtsWaiter::updateAndApply(ChannelData *channel, qint32 pts, qint32 count, const MTPUpdates &updates) {
...
bool PtsWaiter::updateAndApply(ChannelData *channel, qint32 pts, qint32 count, const MTPUpdate &update) {
...
bool PtsWaiter::updateAndApply(ChannelData *channel, qint32 pts, qint32 count) {
...
QString joinList(const QStringList &list, const QString &sep) {
	...
	qint32 l = list.size(), s = sep.size() * (l - 1);
	for (qint32 i = 0; i < l; ++i) {
		s += list.at(i).size();
	}
	result.reserve(s);
	result.append(list.at(0));
	for (qint32 i = 1; i < l; ++i) {
...
QString saveFileName(const QString &title, const QString &filter, const QString &prefix, QString name, bool savingAs, const QDir &dir) {
	...
	qint32 extPos = name.lastIndexOf('.');
	if (extPos >= 0) {
...
VoiceData::~VoiceData() {
	if (!waveform.isEmpty() && waveform.at(0) == -1 && waveform.size() > qint32(sizeof(TaskId))) {
		TaskId taskId = 0;
...
void DocumentData::setattributes(const QVector<MTPDocumentAttribute> &attributes) {
	for (qint32 i = 0, l = attributes.size(); i < l; ++i) {
		...
		case mtpc_documentAttributeAudio: {
			...
			if (voice()) {
				...
				uchar wavemax = 0;
				for (qint32 i = 0, l = waveform.size(); i < l; ++i) {
...
qint32 DocumentData::loadOffset() const {
...
VoiceWaveform documentWaveformDecode(const QByteArray &encoded5bit) {
	...
	// And then we read a quint16 starting from that byte to guarantee to get all of those 5 bits.
	//
	// BUT! if it is the last byte we have, we're not allowed to read a quint16 starting with it.
...
QByteArray documentWaveformEncode5bit(const VoiceWaveform &waveform) {
	...
	// as a quint16 without overflowing, even the byte with index "bytesCount - 1".
...
void DocumentData::setRemoteLocation(qint32 dc, quint64 access) {
	_dc = dc;
	_access = access;
	if (isValid()) {
		if (_location.check()) {
