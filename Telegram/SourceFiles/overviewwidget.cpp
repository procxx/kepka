...
qint32 OverviewInner::migratedIndexSkip() const {
	return (_migrated && _history->overviewLoaded(_type)) ? _migrated->overview(_type).size() : 0;
}

void OverviewInner::fixItemIndex(qint32 &current, MsgId msgId) const {
	if (!msgId) {
		current = -1;
	} else {
		qint32 l = _items.size();
...
bool OverviewInner::itemHasPoint(MsgId msgId, qint32 index, qint32 x, qint32 y) const {
...
qint32 OverviewInner::itemHeight(MsgId msgId, qint32 index) const {
	if (_type == OverviewPhotos || _type == OverviewVideos) {
		return _rowWidth;
	}
...
void OverviewInner::moveToNextItem(MsgId &msgId, qint32 &index, MsgId upTo, qint32 delta) const {
	fixItemIndex(index, msgId);
...
void OverviewInner::repaintItem(MsgId itemId, qint32 itemIndex) {
...
void OverviewInner::addSelectionRange(qint32 selFrom, qint32 selTo, History *history) {
	if (selFrom < 0 || selTo < 0) return;
	for (qint32 i = selFrom; i <= selTo; ++i) {
...
QPoint OverviewInner::mapMouseToItem(QPoint p, MsgId itemId, qint32 itemIndex) {
	fixItemIndex(itemIndex, itemId);
	...
	if (_type == OverviewPhotos || _type == OverviewVideos) {
		...
		p.setX(p.x() - qint32(col * w) - st::overviewPhotoSkip);
...
void OverviewInner::paintEvent(QPaintEvent *e) {
	...
	if (_type == OverviewPhotos || _type == OverviewVideos) {
		...
		for (qint32 row = rowFrom; row < rowTo; ++row) {
			...
			for (qint32 col = 0; col < _photosInRow; ++col) {
				qint32 i = row * _photosInRow + col;
				if (i < 0) continue;
				if (i >= count) break;
...
void OverviewInner::onUpdateSelected() {
	...
	if (_type == OverviewPhotos || _type == OverviewVideos) {
		double w = (double(_width - st::overviewPhotoSkip) / _photosInRow);
		qint32 col = qint32((m.x() - (st::overviewPhotoSkip / 2)) / w), vsize = (_rowWidth + st::overviewPhotoSkip);
		qint32 row = qint32((m.y() - _marginTop - (st::overviewPhotoSkip / 2)) / vsize);
		if (col < 0) col = 0;
		if (row < 0) row = 0;
		bool upon = true;

		qint32 count = _items.size(), i = row * _photosInRow + col;
		if (i < 0) {
			i = 0;
			upon = false;
		}
	...
	qint32 oldMousedItemIndex = _mousedItemIndex;
...
void OverviewInner::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
	...
	qint32 isUponSelected = 0, hasSelected = 0;
...
qint32 OverviewInner::resizeToWidth(qint32 nwidth, qint32 scrollTop, qint32 minHeight, bool force) {
...
void OverviewInner::mediaOverviewUpdated() {
	...
	recountMargins();
	qint32 newHeight = _marginTop + _height + _marginBottom, deltaHeight = newHeight - height();
	if (deltaHeight) {
...
void OverviewInner::itemRemoved(HistoryItem *item) {
	...
	if (j != _layoutItems.cend()) {
		qint32 index = _items.indexOf(j.value());
		if (index >= 0) {
			_items.remove(index);
		}
		delete j.value();
...
Overview::Layout::AbstractItem *OverviewInner::layoutPrepare(const QDate &date, bool month) {
	qint32 key = date.year() * 100 + date.month();
...
qint32 OverviewInner::setLayoutItem(qint32 index, Overview::Layout::AbstractItem *item, qint32 top) {
	if (_items.size() > index) {
		_items[index] = item;
	} else {
		_items.push_back(item);
	}
	qint32 h = item->resizeGetHeight(_rowWidth);
...
void OverviewWidget::scrollBy(qint32 add) {
	if (_scroll->isHidden()) {
...
qint32 OverviewWidget::lastWidth() const {
	return width();
}

qint32 OverviewWidget::lastScrollTop() const {
...
qint32 OverviewWidget::countBestScroll() const {
