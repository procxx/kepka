...
void paintRowDate(Painter &p, const QDateTime &date, QRect &rectForName, bool active, bool selected) {
	...
	p.drawText(rectForName.left() + rectForName.width() + st::dialogsDateSkip,
	           rectForName.top() + st::msgNameFont->height - st::msgDateFont->descent, dt);
}

template <typename PaintItemCallback, typename PaintCounterCallback>
void paintRow(Painter &p, const RippleRow *row, History *history, not_null<PeerData *> from, HistoryItem *item,
              Data::Draft *draft, QDateTime date, int fullWidth, bool active, bool selected, bool onlyBackground,
              TimeMs ms, PaintItemCallback paintItemCallback, PaintCounterCallback paintCounterCallback) {
	...
	if (draft) {
		...
		if (history->isPinnedDialog()) {
			auto &icon =
			    (active ? st::dialogsPinnedIconActive : (selected ? st::dialogsPinnedIconOver : st::dialogsPinnedIcon));
		...
		if (!history->paintSendAction(p, nameleft, texttop, availableWidth, fullWidth, color, ms)) {
			if (history->cloudDraftTextCache.isEmpty()) {
				...
				auto draftText = lng_dialogs_text_with_from(lt_from_part, draftWrapped, lt_message,
				                                            TextUtilities::Clean(draft->textWithTags.text));
				history->cloudDraftTextCache.setText(st::dialogsTextStyle, draftText, _textDlgOptions);
			}
			p.setPen(active ? st::dialogsTextFgActive : (selected ? st::dialogsTextFgOver : st::dialogsTextFg));
			p.setTextPalette(active ? st::dialogsTextPaletteDraftActive :
			                          (selected ? st::dialogsTextPaletteDraftOver : st::dialogsTextPaletteDraft));
	...
	auto sendStateIcon = ([draft, item, active, selected]() -> const style::icon * {
		if (draft) {
			if (draft->saveRequestId) {
				return &(active ? st::dialogsSendingIconActive :
				                  (selected ? st::dialogsSendingIconOver : st::dialogsSendingIcon));
			}
		} else if (item && !item->isEmpty() && item->needCheck()) {
			if (item->id > 0) {
				if (item->unread()) {
					return &(active ? st::dialogsSentIconActive :
					                  (selected ? st::dialogsSentIconOver : st::dialogsSentIcon));
				}
				return &(active ? st::dialogsReceivedIconActive :
				                  (selected ? st::dialogsReceivedIconOver : st::dialogsReceivedIcon));
			}
			return &(active ? st::dialogsSendingIconActive :
			                  (selected ? st::dialogsSendingIconOver : st::dialogsSendingIcon));
		}
	...
	if (from == history->peer && from->isVerified()) {
		...
		icon->paint(p, rectForName.topLeft() + QPoint(std::min(from->dialogName().maxWidth(), rectForName.width()), 0),
		            fullWidth);
	}
...
public:
	...
	style::color bg[6] = {st::dialogsUnreadBg,      st::dialogsUnreadBgOver,      st::dialogsUnreadBgActive,
...
const style::icon *ChatTypeIcon(PeerData *peer, bool active, bool selected) {
	if (peer->isChat() || peer->isMegagroup()) {
		...
		return &(active ? st::dialogsChannelIconActive :
		                  (selected ? st::dialogsChannelIconOver : st::dialogsChannelIcon));
	}
	return nullptr;
}
...
void paintUnreadBadge(Painter &p, const QRect &rect, const UnreadBadgeStyle &st) {
	...
	if (badgeData->left[index].isNull()) {
		...
		badgeData->left[index] =
		    App::pixmapFromImageInPlace(colorizeCircleHalf(badgeData, imgsize, imgsizehalf, 0, bg));
		badgeData->right[index] =
		    App::pixmapFromImageInPlace(colorizeCircleHalf(badgeData, imgsize, imgsizehalf, imgsize - imgsizehalf, bg));
	}
...
void RowPainter::paint(Painter &p, const Row *row, int fullWidth, bool active, bool selected, bool onlyBackground,
                       TimeMs ms) {
	...
	paintRow(
	    p, row, history, from, item, cloudDraft, displayDate(), fullWidth, active, selected, onlyBackground, ms,
	    [&p, fullWidth, active, selected, ms, history, unreadCount](int nameleft, int namewidth, HistoryItem *item) {
		    auto availableWidth = namewidth;
		    ...
		    if (displayMentionBadge && unreadCount == 1 && item && item->isMediaUnread() && item->mentionsMe()) {
...
void RowPainter::paint(Painter &p, const FakeRow *row, int fullWidth, bool active, bool selected, bool onlyBackground,
                       TimeMs ms) {
	auto item = row->item();
	...
	paintRow(p, row, history, from, item, nullptr, item->date, fullWidth, active, selected, onlyBackground, ms,
	         [&p, row, active, selected](int nameleft, int namewidth, HistoryItem *item) {
		         int lastWidth = namewidth,
		             texttop = st::dialogsPadding.y() + st::msgNameFont->height + st::dialogsSkip;
		         item->drawInDialog(p, QRect(nameleft, texttop, lastWidth, st::dialogsTextFont->height), active,
		                            selected, HistoryItem::DrawInDialog::WithoutSender, row->_cacheFor, row->_cache);
	         },
	         [] {});
}
...
void paintImportantSwitch(Painter &p, Mode current, int fullWidth, bool selected, bool onlyBackground) {
	...
	int textBaseline =
	    unreadTop + (st::dialogsUnreadHeight - st::dialogsUnreadFont->height) / 2 + st::dialogsUnreadFont->ascent;
